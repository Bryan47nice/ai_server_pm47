const dict = {
    zh: {
        ...
        't-npi-dvt-badge': 'DVT',
        't-npi-dvt-period': 'M-12 ~ M-8',
        't-npi-dvt-title': '設計驗證與關鍵零件認証',
        't-npi-dvt-task-1': '追蹤 EE / ME / Thermal 設計進度，主持每週 Issue Review 會議',
        't-npi-dvt-task-2': '確認 GPU / CPU / HBM 等關鍵零件的 AVL 認証狀態',
        't-npi-dvt-task-3': '下單 Long Lead Time 零件並建立交期追蹤看板',
        't-npi-dvt-task-4': '制定 DVT 測試計畫，預約 SIT 實驗室資源',
        't-npi-dvt-bn-1': 'GPU 等主晶片 Lead Time 超過 20 週，樣品到位嚴重延遲',
        't-npi-dvt-bn-2': '散熱設計不達標，需重新 Layout，延期 6~8 週',
        't-npi-dvt-bn-3': 'EE 與 ME 設計邊界衝突，需多輪協調消耗大量時程',
        't-npi-dvt-risk': '上游晶片廠（如 NVIDIA）臨時更新規格 (Spec Rev)，強制觸發 BOM 修改，牽動整條 Critical Path',

        't-npi-evt-badge': 'EVT',
        't-npi-evt-period': 'M-8 ~ M-5',
        't-npi-evt-title': 'BOM 凍結與工程全面驗證',
        't-npi-evt-task-1': '主導 BOM Freeze 會議，鎖定工程確認版本',
        't-npi-evt-task-2': '協調 FAE 排入燒機 (Burn-in) 與相容性測試',
        't-npi-evt-task-3': '追蹤 EVT Issue List，設定 Owner 與 ETA，強力推動收斂',
        't-npi-evt-task-4': '安排客戶 Pre-qualification 送樣與技術文件準備',
        't-npi-evt-bn-1': '客戶臨時追加規格（Scope Creep），BOM Freeze 一再遞延',
        't-npi-evt-bn-2': '測試 Fail 反覆，PCBA 需多輪重工，迭代週期拉長',
        't-npi-evt-bn-3': '多個 Issue 相互依賴，形成 Critical Path 死結，需 PJM 強力拆解',
        't-npi-evt-risk': 'BOM 未準時凍結，導致 PVT 備料失準，量產備料成本暴增且交期延誤',
    ...
    en: {
        ...
        't-npi-dvt-badge': 'DVT',
        't-npi-dvt-period': 'M-12 ~ M-8',
        't-npi-dvt-title': 'Design Verification & Key Component Qualification',
        't-npi-dvt-task-1': 'Track EE / ME / Thermal design progress; chair weekly Issue Review',
        't-npi-dvt-task-2': 'Confirm AVL qualification status for GPU / CPU / HBM',
        't-npi-dvt-task-3': 'Place orders for long lead-time parts and build delivery tracking board',
        't-npi-dvt-task-4': 'Draft DVT test plan and reserve SIT lab resources',
        't-npi-dvt-bn-1': 'GPU lead time exceeds 20 weeks — sample delivery severely delayed',
        't-npi-dvt-bn-2': 'Thermal design falls short — requires re-layout, adding 6–8 weeks',
        't-npi-dvt-bn-3': 'EE/ME boundary conflicts require multi-round coordination, consuming the schedule',
        't-npi-dvt-risk': 'Upstream chip vendor (e.g., NVIDIA) releases a Spec Rev, forcing a BOM change that impacts the entire Critical Path',

        't-npi-evt-badge': 'EVT',
        't-npi-evt-period': 'M-8 ~ M-5',
        't-npi-evt-title': 'BOM Freeze & Full Engineering Validation',
        't-npi-evt-task-1': 'Lead BOM Freeze meeting to lock the engineering-confirmed revision',
        't-npi-evt-task-2': 'Coordinate FAE for Burn-in and compatibility testing',
        't-npi-evt-task-3': 'Track EVT Issue List — assign Owner & ETA, drive aggressive closure',
        't-npi-evt-task-4': 'Arrange customer Pre-qualification sample shipment and technical documentation',
        't-npi-evt-bn-1': 'Customer adds last-minute requirements (Scope Creep), repeatedly delaying BOM Freeze',
        't-npi-evt-bn-2': 'Repeated test failures require multi-pass PCBA rework, extending iteration cycles',
        't-npi-evt-bn-3': 'Interdependent issues create a Critical Path deadlock — requires PJM intervention to break',
        't-npi-evt-risk': 'Failure to freeze BOM on time causes PVT material planning misalignment, inflating procurement costs and delaying MP',
