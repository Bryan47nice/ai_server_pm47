...
本專案為一款專為 B2B 伺服器代工廠 (ODM) 產品企劃與前端業務打造的 **「BI 商業智慧級決策與銷售賦能系統」**。不僅提供精準的 TCO (總擁有成本) 試算，更具備 **多情境方案比較 (Scenario Comparison)** 與 **AI 跨方案決策報告** 功能。協助業務以數據為基底，具象化硬體架構轉換的長期財務 ROI，展現「幫客戶做決策，而不只是給數據」的高階商業價值。
...
### 📊 Module B: BI 商業智慧多情境比較 (Scenario Comparison)
...
### 🪄 Module C: 業務賦能與極致防呆 (Sales Enablement & UX)
...
### 📸 Module D: 企業級輸出與防禦渲染引擎 (Advanced Export Engine)
* **鐵壁 A4 PDF 匯出：** 針對 `@media print` 進行底層 CSS 區塊流 (Block Flow) 重構，廢棄彈性佈局，精準鎖定各層高度與絕對定位頁尾，徹底解決列印重疊與斷頁問題。
* **一鍵高畫質 PNG 截圖：** 針對特定 DOM 區塊實裝前端渲染快門。克服 `html2canvas` 痛點，實裝「強制捲動歸零 (Scroll Reset)」防切邊，並於截圖瞬間注入動態呼吸畫框 (Dynamic Padding) 與拔除陰影地雷，確保產出完美置中的簡報級圖卡。
* **智慧防呆攔截彈窗：** 若使用者在「決策報告未生成」時試圖截圖，系統將手動彈出精美的 Tailwind 提示視窗，建議「產出報告並匯出」，大幅提升 AI 總結功能的使用率。

### 🌐 Module E: 國際化與純淨架構 (i18n & Clean Architecture)
* **i18n 中英雙語徹底解耦：** 全站支援即時中英切換（含圖表標籤、Tooltip 與防呆提示）。清償硬編碼技術債，將所有字串抽離至獨立的 `i18n_dict.js` 字典檔中，利用 DOM 映射自動替換。
...
* ⚙️ `tco_engine.js` (Controller & Logic): 核心運算大腦。封裝硬體功耗換算、Chart.js 多實體渲染、情境記憶體陣列 (`scenarios`) 管理、以及 PDF/PNG 複雜的非同步匯出與 DOM 攔截邏輯。
* 🌐 `i18n_dict.js` (Data - L10n): 獨立語系資料字典。集中管理 50+ 組全域中英文字串，未來擴充第三語言無需更動核心代碼。
...
* `< 40 kW/Rack:` 🟢 推薦氣冷方案 (Air Cooling)
* `40 - 80 kW/Rack:` 🟡 氣冷極限 (RDHX)
* `> 80 kW/Rack:` 🔴 強制液冷方案 (Must use DLC)
